...
from sour.map.Cube import genfaceverts, notouchingface, touchingface
from sour.map.dimension import opposite, dimension, dimcoord
from sour.common.constants import empty_material_types, F_SOLID
from sour.map.insideface import insideface
from sour.map.facevec import facevec
from sour.map.occludesface import occludesface
from sour.map.genfacevecs import genfacevecs
from sour.map.faceedges import faceedges
from sour.map.neighbourcube import neighbourcube
...
def visibletris(c, cube_map, orient, x, y, z, size, nmat, matmask):
    ...
    if nsize > size or (nsize == size and o.children == 0):
        ...
        if o.isentirelysolid() or (
            touchingface(o, opp) and faceedges(o, opp) == F_SOLID
        ):
            return vis & notouch
