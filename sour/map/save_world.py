...
from sour.map.dimension import dimension
from sour.map.Cube import faceconvexity
from sour.map.visibletris import visibletris
...
def savevslots(f, vslots):
    ...
    for i in xrange(numvslots):
        ...
        while 1:
            ...
            while 1:
                vs = vs.next
                if vs is None or vs.index < numvslots:
                    break
            if vs is None:
                break
                    ...
                    if surfmask & (1 << j):
                        ...
                        if numverts:
                            if c[i].merged & (1 << j):
                                ...
                                vis = visibletris(
                                    c[i], cube_map, j, co.x, co.y, co.z, size
                                )
