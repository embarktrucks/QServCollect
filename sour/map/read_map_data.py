...
from sour.map.encodenormal import encodenormal
from sour.map.newcubeext import newcubeext
from sour.map.SurfaceInfo import SurfaceInfo
from sour.map.VertInfo import VertInfo
from sour.map.Cube import Cube, setsurfaces, genfaceverts, newcubes
from sour.map.facecoords import facecoords
from sour.common.constants import (
    layer_types,
    lighting_types,
    LM_PACKW,
    USHRT_MAX,
...
from sour.common.ivec import ivec, C, R
from sour.common.bvec import bvec
from sour.map.errors import OctaError
from sour.map.convertoldmaterial import convertoldmaterial
from sour.map.SurfaceCompat import SurfaceCompat
from sour.map.NormalsCompat import NormalsCompat
from sour.map.MergesCompat import MergesCompat
from sour.common.vec import vec
from sour.map.load_vslots import load_vslots
from sour.map.CubeMap import CubeMap


def convertoldsurfaces(
    c, co, size, srcsurfs, hassurfs, normals, hasnorms, merges, hasmerges
):
                ...
                for k in xrange(4):
                    ...
                    if uselms:
                        ...
                        dv.u = ushort(
                            floor(
                                clamp(
                                    (u) * float(USHRT_MAX + 1) / LM_PACKW + 0.5,
                                    0.0,
                                    float(USHRT_MAX),
                                )
                            )
                        ...
                        dv.v = ushort(
                            floor(
                                clamp(
                                    (v) * float(USHRT_MAX + 1) / LM_PACKH + 0.5,
                                    0.0,
                                    float(USHRT_MAX),
                                )
                            )
...
def loadc(version, f, cube, co, size):
    ...
    if version < 7:
        ...
        if mask & 0x80:
            ...
            if version < 27:
                matconv = [
                    ...
                    empty_material_types.MAT_GLASS | empty_material_types.MAT_CLIP,
                    empty_material_types.MAT_NOCLIP,
                    empty_material_types.MAT_LAVA | empty_material_types.MAT_DEATH,
                    empty_material_types.MAT_GAMECLIP,
                    empty_material_types.MAT_DEATH,
                ]
                cube.material = (
                    matconv[mat] if mat < len(matconv) else empty_material_types.MAT_AIR
                )
            else:
        ...
        if hassurfs or hasnorms or hasmerges:
            convertoldsurfaces(
                cube, co, size, surfaces, hassurfs, normals, hasnorms, merges, hasmerges
            )
    else:
        if octsav & 0x40:
            if version <= 32:
            ...
            for i in xrange(6):
                if surfmask & (1 << i):
                    ...
                    if hasxyz:
                        ...
                        bias = -n.dot(
                            ivec(v[0]).mul(size).add(ivec(co).mask(0xFFF).shl(3))
                        )
                    else:
                        if layerverts < 4:
