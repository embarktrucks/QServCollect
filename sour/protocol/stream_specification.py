    ...
    def read(self, stream_object, type_method_mapping, game_state={}):
        ...
        for field in self.fields:
            if isinstance(field, ConditionalFieldCollection) or isinstance(
                ...
                field_data = field.read(
                    stream_object, type_method_mapping, game_state=game_state
                )
...
class TerminatedFieldCollection(object):
    ...
    def read(self, stream_object, type_method_mapping, game_state={}):
        ...
        _, term_value = self.terminator_field.read(
            stream_object, type_method_mapping, peek=True, game_state=game_state
        )
        ...
        _, term_value = self.terminator_field.read(
            stream_object, type_method_mapping, peek=False, game_state=game_state
        )
...
class ConditionalFieldCollection(object):
    ...
    def read(self, stream_object, type_method_mapping, game_state={}):
        ...
        if predicate_result:
            return self.consequent.read(
                stream_object, type_method_mapping, game_state=game_state
            )
        elif self.alternative is not None:
            return self.alternative.read(
                stream_object, type_method_mapping, game_state=game_state
            )
        else:
            return {}
...
class SwitchField(object):
    ...
    def read(self, stream_object, type_method_mapping, game_state={}):
        ...
        for case in self.cases:
            ...
            if predicate_result:
                return case.consequent.read(
                    stream_object, type_method_mapping, game_state=game_state
                )
        if self.default is not None:
            return self.default.read(
                stream_object, type_method_mapping, game_state=game_state
            )
        else:
            return {}
...
class MessageType(FieldCollection):
    ...
    def read(self, stream_object, type_method_mapping, game_state={}):
        ...
        try:
            return (
                self.message_name,
                FieldCollection.read(
                    self, stream_object, type_method_mapping, game_state=game_state
                ),
            )
        except:
            logger.error(
...
class CustomMessageType(object):
    ...
    def read(self, stream_object, type_method_mapping, game_state={}):
        ...
        try:
            return (
                self.message_name,
                self.func(stream_object, type_method_mapping, game_state=game_state),
            )
        except:
            logger.error(
                "Exception occurred in MessageType '{}'".format(self.message_name)
    ...
    def read(self, raw_stream, initial_state, game_state={}):
        ...
        try:
            ...
            while not stream_object.empty():
                message_type_id = self.type_method_mapping[self.message_type_id_type](
                    stream_object
                )
                ...
                if message_type_id in self.message_types.keys():
                    message_name, datum = self.message_types[message_type_id].read(
                        stream_object, self.type_method_mapping, game_state
                    )
                    datum.update(state)
...
class StreamContainerType(StreamSpecification):
    ...
    def read(self, stream_object, type_method_mapping, initial_state, game_state={}):
        ...
        _, stream_length = self.length_field.read(
            stream_object, type_method_mapping, game_state=game_state
        )
