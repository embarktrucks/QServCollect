from sour.common.utils.enum import enum
...
cs_id_types = enum(
    "ID_VAR", "ID_FVAR", "ID_SVAR", "ID_COMMAND", "ID_CCOMMAND", "ID_ALIAS"
)

client_states = enum(
    "CS_ALIVE", "CS_DEAD", "CS_SPAWNING", "CS_LAGGED", "CS_EDITING", "CS_SPECTATOR"
)
...
shader_param_types = enum(
    "SHPARAM_LOOKUP", "SHPARAM_VERTEX", "SHPARAM_PIXEL", "SHPARAM_UNIFORM"
)
vslot_types = enum(
    "VSLOT_SHPARAM",
    "VSLOT_SCALE",
...
octa_save_types = enum(
    "OCTSAV_CHILDREN", "OCTSAV_EMPTY", "OCTSAV_SOLID", "OCTSAV_NORMAL", "OCTSAV_LODCUBE"
)
...
empty_material_types = enum(  # cube empty-space materials
    ...
    MAT_WATER=1
    << 2,  # MATF_VOLUME_SHIFT, # fill with water, showing waves at the surface
...
class GunInfo(object):
    sound, attackdelay, damage, spread, projspeed, kickamount, gunrange, rays, hitpush, exprad, ttl = (
        [0] * 11
    )
...
guns = [
    ...
    GunInfo(
        sounds.S_RLFIRE, 800, 120, 0, 320, 10, 1024, 1, 160, 40, 0, "rocketlauncher"
    ),
    GunInfo(sounds.S_RIFLE, 1500, 100, 0, 0, 30, 2048, 1, 80, 0, 0, "rifle"),
    GunInfo(
        sounds.S_FLAUNCH, 500, 90, 0, 200, 10, 1024, 1, 250, 45, 1500, "grenadelauncher"
    ),
...
mastermodes = enum(
    "MM_OPEN", "MM_VETO", "MM_LOCKED", "MM_PRIVATE", "MM_PASSWORD", MM_AUTH=-1
)

disconnect_types = enum(
    "DISC_NONE",
    "DISC_EOP",
    "DISC_LOCAL",
    "DISC_KICK",
